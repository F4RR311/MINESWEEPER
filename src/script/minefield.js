...
class Minefield {
    ...
    initialize = function (cfg) {
        ...
        grid.style.gridTemplateColumns = `repeat(${cfg.cols}, 32px)`;
        grid.style.gridTemplateRows = `repeat(${cfg.rows}, 32px)`;
        cfg.parent.appendChild(grid);

        cfg.mines.set(cfg.mineCount, 0, cfg.mineCount);
        cfg.timer.setValue(0);
        if (this.interval) {
    ...
    floodFill(x, y) {
        ...
        do {
            ...
            if (cell.info > -1 && cell.state != DISCOVERED) {
